...
| 1 validate | the gate. Nothing below runs unless every document validates; on a full release the validator is also given `PROVENANCE_FLOOR` — a literal in the script, set by a human, naming how many items must carry a non-null `nge:` value (#32). `check_collection_metadata` and `check_citation_premise` also run here: the licence, the six providers, both links and `sci:citation` asserted verbatim, and every item's `nge:landcover_collection` **and** `nge:landcover_stac_url` checked against what the citation attributes — the id and the host can move independently, and a host move alone would change the licensor (#47) |
