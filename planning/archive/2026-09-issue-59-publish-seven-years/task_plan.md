...
- [x] `WSG=necr Rscript scripts/test_pipeline.R`
- [x] Gate before publishing (as Phase 1 — pixel/geometry/RAT identity, not byte equality)
- [x] `bash scripts/catalogue_release.sh --only necr_ch_ff04`
- [x] Read back; `nge:landcover_key = sha256:1635efbfe58ec14ff802480ea07f47a2d6a43ab195d60881160ed32b3d94571a`

## Phase 3: lnth_ch_ff04

- [x] `WSG=lnth Rscript scripts/test_pipeline.R`
- [x] Gate before publishing (as Phase 1 — pixel/geometry/RAT identity, not byte equality)
- [x] `bash scripts/catalogue_release.sh --only lnth_ch_ff04`
- [x] Read back; `nge:landcover_key = sha256:a7cd994b621ed5aa3c05c62698013a7bc7a57479bb5471da72bf4c9842403e7c`

## Phase 4: kotl_bt_ff04

- [x] `WSG=kotl Rscript scripts/test_pipeline.R`
- [x] Gate before publishing (as Phase 1 — pixel/geometry/RAT identity, not byte equality)
- [x] `bash scripts/catalogue_release.sh --only kotl_bt_ff04`
- [x] Read back; `nge:landcover_key = sha256:3c70e523394efb82632215d1b3cb4661c01e20e2df4676796ba6c7236be44af5`
...
- [x] Re-read all 23 live items and diff against `baseline_live.json`. Assert the **19 untouched items are identical** in asset key set, every `file:checksum`, every `file:size` and every `nge:` property — and that the four moved in exactly the predicted places and nowhere else.
- [x] Fetch every published `classified_*` href for the four over HTTP — `gdalinfo /vsicurl/…` on one to confirm the RAT survived, a ranged read on the rest (drift#62's acceptance).
